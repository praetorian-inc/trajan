...
func (p *GitHubParser) Parse(data []byte) (*NormalizedWorkflow, error) {
	// Unmarshalled twice: yaml.Node carries line numbers, the typed struct carries structure.
	var node yaml.Node
...
func (p *GitHubParser) walkNode(node *yaml.Node, path string, lineMap map[string]int) {
	...
	case yaml.MappingNode:
		...
		for i := 0; i < len(node.Content); i += 2 {
			...
			// The key's line, not the value's.
...
// A ${{ matrix.os }} runs-on is resolved against the strategy matrix; an
// unresolvable expression is treated as self-hosted.
func (j GitHubJob) IsSelfHostedRunner() bool {
	// A reusable-workflow caller has no runner of its own; runs-on belongs to the callee.
	if j.Uses != "" {
		return false
	}
