...
// runLocalScan scans local workflow files without contacting the Azure DevOps API.
func runLocalScan(cmd *cobra.Command, path string) error {
	verbose := cmdutil.GetVerbose(cmd)
	output := cmdutil.GetOutput(cmd)

	repoSlug := "local:" + filepath.Base(path)
	workflows, err := localwalk.Walk(platforms.PlatformAzureDevOps, path, repoSlug)
	if err != nil {
		return fmt.Errorf("walking local path: %w", err)
	}

	if verbose {
		fmt.Fprintf(os.Stderr, "Found %d local Azure DevOps workflow(s) in %s\n", len(workflows), path)
	}

	allPlugins := registry.GetDetectionsForPlatform(platforms.PlatformAzureDevOps)
	...
	if capabilities != "" {
		filteredFindings, err := cmdutil.FilterFindingsByADOCapabilities(findings, capabilities)
		if err != nil {
			return fmt.Errorf("filtering by capabilities: %w", err)
		}
		findings = filteredFindings
